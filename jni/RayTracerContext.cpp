...
void RayTracerContext::updateTriangleCenter(const int &tri_idx) {
	Vertex v1, v2, v3;
...
int RayTracerContext::addVertex(const float &_x, const float &_y, const float &_z) {
	...
	// Expand the scene AABB.
    sceneBox.contain(vertArray[numVert-1]);

	return numVert - 1;
}

int RayTracerContext::addTriangle(const int &v1, const int &v2, const int &v3, const int &mat_idx) {
